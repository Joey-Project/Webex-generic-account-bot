...
impl ConfigCommandsConfig {
    pub fn validate(&self) -> Result<()> {
        ...
        {
            return Err(anyhow!(
                "config_commands command {command:?} is not deployable until recoverable activation is enabled; only status and pull are supported"
            ));
        }
...
"#,
    ...
    #[test]
    fn accepts_pull_but_rejects_activation_commands() {
        let mut pull = valid_config();
        pull.allowed_commands.push(ConfigCommand::Pull);
